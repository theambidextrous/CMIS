  ...
  <div class="col-lg-12">
    <div class="panel panel-default">
      ...
      <div class="panel-body"> 
        ...
        <a class="btn btn-primary" href="admin.php?tab=10&task=edit">Global Settings</a>
      </div>
      <!-- /.panel-body --> 
    </div>
    <!-- /.panel-default --> 
  ...
  <div class="col-lg-12">
    <div class="panel panel-default">
      ...
      <div class="panel-body">
		...
		$FIELDS['DateStart'] = db_fixdate($_POST['DateStart']);
		$FIELDS['DateEnd'] = db_fixdate($_POST['DateEnd']);
		...
		if(isset($_POST['Add'])){
			...
			if(!empty($FIELDS['DateStart'])){
				...
				if($check->is_date($SplitDateStart[0],$SplitDateStart[1],$SplitDateStart[2])){
					$FIELDS['DateStart'] = db_fixdate($FIELDS['DateStart']);// YYYY-mm-dd
				}else{
			...
			if(!empty($FIELDS['DateEnd'])){
				...
				if($check->is_date($SplitDateEnd[0],$SplitDateEnd[1],$SplitDateEnd[2])){
					$FIELDS['DateEnd'] = db_fixdate($FIELDS['DateEnd']);// YYYY-mm-dd
				}else{
			...
			if(!empty($FIELDS['RegDateOpen'])){
				...
				if($check->is_date($SplitRegDateOpen[0],$SplitRegDateOpen[1],$SplitRegDateOpen[2])){
					$FIELDS['RegDateOpen'] = db_fixdate($FIELDS['RegDateOpen']);// YYYY-mm-dd
				}else{
			...
			if(!empty($FIELDS['RegDateClosed'])){
				...
				if($check->is_date($SplitRegDateClosed[0],$SplitRegDateClosed[1],$SplitRegDateClosed[2])){
					$FIELDS['RegDateClosed'] = db_fixdate($FIELDS['RegDateClosed']);// YYYY-mm-dd
				}else{
			...
			if(sizeof($ERRORS) > 0){
				...
				$sql_insert = sprintf("INSERT INTO `".DB_PREFIX."academic_yrs` (`AcYear`,`AcName`,`AcTerm`,`DateStart`,`DateEnd`,`RegDateOpen`,`RegDateClosed`,`RegDetails`) VALUES ('%s','%s','%s','%s','%s','%s','%s','%s')", $FIELDS['AcYear'],$FIELDS['AcName'],$FIELDS['AcTerm'],$FIELDS['DateStart'],$FIELDS['DateEnd'],$FIELDS['RegDateOpen'],$FIELDS['RegDateClosed'],$FIELDS['RegDetails']);
		...
		if(isset($_POST['Edit']) && !empty($eid)){
			...
			if(!empty($FIELDS['DateStart'])){
				$SplitDateStart = explode('/', $FIELDS['DateStart']);// Split date by '/'
				//checkdate($month, $day, $year)
				if($check->is_date($SplitDateStart[1],$SplitDateStart[0],$SplitDateStart[2])){
					$FIELDS['DateStart'] = db_fixdate($FIELDS['DateStart']);// YYYY-mm-dd
				}else{
					$ERRORS['TermPeriod'] = "Valid start date is required";
				}
			...
			if(!empty($FIELDS['DateEnd'])){
				$SplitDateEnd = explode('/', $FIELDS['DateEnd']);// Split date by '/'
				//checkdate($month, $day, $year)
				if($check->is_date($SplitDateEnd[1],$SplitDateEnd[0],$SplitDateEnd[2])){
					$FIELDS['DateEnd'] = db_fixdate($FIELDS['DateEnd']);// YYYY-mm-dd
				}else{
					$ERRORS['TermPeriod'] = "Valid end date is required";
				}
			...
			if(!empty($FIELDS['RegDateOpen'])){
				$SplitRegDateOpen = explode('/', $FIELDS['RegDateOpen']);// Split date by '/'
				//checkdate($month, $day, $year)
				if($check->is_date($SplitRegDateOpen[1],$SplitRegDateOpen[0],$SplitRegDateOpen[2])){
					$FIELDS['RegDateOpen'] = db_fixdate($FIELDS['RegDateOpen']);// YYYY-mm-dd
				}else{
					$ERRORS['RegWindow'] = "Valid start date is required";
				}
			...
			if(!empty($FIELDS['RegDateClosed'])){
				$SplitRegDateClosed = explode('/', $FIELDS['RegDateClosed']);// Split date by '/'
				//checkdate($month, $day, $year)
				if($check->is_date($SplitRegDateClosed[1],$SplitRegDateClosed[0],$SplitRegDateClosed[2])){
					$FIELDS['RegDateClosed'] = db_fixdate($FIELDS['RegDateClosed']);// YYYY-mm-dd
				}else{
					$ERRORS['RegWindow'] = "Valid end date is required";
				}
			...
			if(sizeof($ERRORS) > 0){
				...
				$sql_update = sprintf("UPDATE `".DB_PREFIX."academic_yrs` SET `AcYear`='%s', `AcName`='%s', `AcTerm`='%s', `DateStart`='%s', `DateEnd`='%s', `RegDateOpen`='%s', `RegDateClosed`='%s', `RegDetails`='%s' WHERE `UID` = %d", $FIELDS['AcYear'],$FIELDS['AcName'],$FIELDS['AcTerm'],$FIELDS['DateStart'],$FIELDS['DateEnd'],$FIELDS['RegDateOpen'],$FIELDS['RegDateClosed'],$FIELDS['RegDetails'],$eid);
		...
		$FIELDS['DateStart'] = !empty($FIELDS['DateStart'])?$FIELDS['DateStart']:fixdatepicker($row['DateStart']);
		$FIELDS['DateEnd'] = !empty($FIELDS['DateEnd'])?$FIELDS['DateEnd']:fixdatepicker($row['DateEnd']);
		$FIELDS['RegDateOpen'] = !empty($FIELDS['RegDateOpen'])?$FIELDS['RegDateOpen']:fixdatepicker($row['RegDateOpen']);
		$FIELDS['RegDateClosed'] = !empty($FIELDS['RegDateClosed'])?$FIELDS['RegDateClosed']:fixdatepicker($row['RegDateClosed']);
        ...
        <div id="myModal" class="modal fade" role="dialog">
          <div class="modal-dialog">
            ...
            <div class="modal-content">
              <form name="academic" method="post" action="admin.php?tab=1&task=add&eid=<?=$eid?>">
                ...
                <div class="row">
                  <div class="col-lg-6">
                    ...
                    <div class="form-group">
                      ...
                      <input type="text" name="AcYear" value="<?=$FIELDS['AcYear']?>" placeholder="Enter academic year e.g. <?=date('Y')?>" class="form-control">
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                      ...
                      <div class="radio">
                        <?php
                        if($FIELDS['AcTerm'] == "Semester"){ $Semester = ' checked="checked"'; }
												//$Semester = $FIELDS['AcTerm'] == "Semester"?$Semester = ' checked="checked"':$Semester = '';
													if($FIELDS['AcTerm'] == "Trimester"){ $Trimester = ' checked="checked"'; }
												//$Trimester = $FIELDS['AcTerm'] == "Trimester"?$Trimester = ' checked="checked"':$Trimester = '';
												?>
                        <label><input<?php $Semester;?> type="radio" name="AcTerm" value="Semester" class="radio">Semester </label>
                        <label><input<?php $Trimester;?> type="radio" name="AcTerm" value="Trimester" class="radio">Trimester </label>
                      </div>
                    </div>
                    <div class="form-group">
                      <label for="termperiod">Term Period:</label>
                      <div class="input-group datepicker-daterange">
                        <input type="text" class="form-control" name="DateStart" value="<?=$FIELDS['DateStart']?>" placeholder="dd/mm/YYYY">
                        <div class="input-group-addon">to</div>
                        <input type="text" class="form-control" name="DateEnd" value="<?=$FIELDS['DateEnd']?>" placeholder="dd/mm/YYYY">
                      </div>
                  ...
                  <div class="col-lg-6">
                    ...
                    <div class="form-group">
                      ...
                      <div class="input-group datepicker-daterange">
                        <input type="text" class="form-control" name="RegDateOpen" value="<?=$FIELDS['RegDateOpen']?>" placeholder="dd/mm/YYYY">
                        <div class="input-group-addon">to</div>
                        <input type="text" class="form-control" name="RegDateClosed" value="<?=$FIELDS['RegDateClosed']?>" placeholder="dd/mm/YYYY">
                      </div>
        ...
        <p><a class="btn btn-primary" href="admin.php?tab=1&task=add">Add New</a> Add the academic year and terms to match with your curriculum calendar.</p>
        ...
        <table width="100%" class="display table table-striped table-bordered table-hover">
          ...
          <tbody>
            <?php
			//Set sql
			$academicSql = "SELECT * FROM `".DB_PREFIX."academic_yrs`";
			...
			if(db_num_rows($resAcademic)>0){
				...
				while($row = db_fetch_array($resAcademic)){
					...
					<td>".fixdatelong($row['RegDateOpen'])." &mdash; ".fixdatelong($row['RegDateOpen'])."</td>
					<td><a href=\"admin.php?tab=1&task=edit&eid=".$row['UID']."\">Edit</a></td>
					</tr>";
					++$counter;
				}
			}
