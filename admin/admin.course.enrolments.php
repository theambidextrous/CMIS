...
if($row['disabledFlag'] == 0){
	echo "<td align=\"center\"><a href=\"admin.php?dispatcher=courseenrolments&enable=1&eid=".$row['UID']."\" title=\"Click to disable ".$row['StudentName']."\"><img border=\"0\" src=\"".IMAGE_FOLDER."/icons/yes.png\" height=\"12\" width=\"12\" alt=\"Disable ".$row['StudentName']."\"></a></td>";
}else{
	echo "<td align=\"center\"><a href=\"admin.php?dispatcher=courseenrolments&enable=0&eid=".$row['UID']."\" title=\"Click to enable ".$row['StudentName']."\"><img border=\"0\" src=\"".IMAGE_FOLDER."/icons/no.png\" height=\"12\" width=\"12\" alt=\"Enable ".$row['StudentName']."\"></a></td>";
}
?>
<?php
if($row['approved'] == 0){
...
?>
<td><a href="admin.php?dispatcher=courseenrolments&task=view&recid=<?=$i ?>&studentID=<?=$row['StudentID'] ?>">Manage</a> | <a href="admin.php?dispatcher=courseenrolments&task=edit&recid=<?=$i ?>&studentID=<?=$row['StudentID'] ?>">Edit</a> | <a href="admin.php?dispatcher=courseenrolments&task=del&recid=<?=$i ?>&studentID=<?=$row['StudentID'] ?>">Delete</a></td>
</tr>        
<?php
}
db_free_result($res);
...
<div id="adv-tab-container">
  ...
  <div class="tab-content">
			...
			if(!empty($row["Courses"])){
				...
				if(!empty($Action) && !empty($row["StudentID"])){
						...
						switch($Action){
								...
								if(!empty($_POST['regUnitIDs'])){
										...
										redirect("?dispatcher=courseenrolments&task=view&recid=$recid&studentID=".$row['StudentID']."#sub-tabs-2");
								}
								break;
								case "drop":
								if(!empty($_POST['regUnitIDs'])){
				...
				<div class="modal fade" id="addUnits" tabindex="-1" role="dialog" aria-labelledby="addUnitsLabel">
					<div class="modal-dialog modal-lg" role="document">
						<form class="form-inline" name="assign-lectures" method="post" action="admin.php?dispatcher=courseenrolments&task=view&recid=<?=$recid?>&studentID=<?=$row["StudentID"]?>&action=add#sub-tabs-2">
				...
				while(list(, $CourseID) = each($CourseIDs)) {
					echo "<tr><td colspan=\"6\"><a class=\"btn btn-sm btn-success\" href=\"admin.php?dispatcher=courseenrolments&task=view&action=approve&recid=$recid&studentID={$row['StudentID']}&courseID=$CourseID\">Approve</a> <a class=\"btn btn-sm btn-danger\" href=\"admin.php?dispatcher=courseenrolments&task=view&action=reject&recid=$recid&studentID={$row['StudentID']}&courseID=$CourseID\">Reject</a> <strong>($CourseID) ".getCourseName($CourseID)."</strong></td></tr>";
    ...
    <div class="quick-nav btn-group">
      <a class="btn btn-default" href="admin.php?dispatcher=courseenrolments&task=add"><i class="fa fa-file-o fa-fw"></i>Add Student</a>
      <a class="btn btn-default" href="admin.php?dispatcher=courseenrolments&task=edit&recid=<?=$recid ?>"><i class="fa fa-pencil-square-o fa-fw"></i>Edit Student</a>
      <a class="btn btn-default" href="admin.php?dispatcher=courseenrolments&task=del&recid=<?=$recid ?>"><i class="fa fa-trash-o fa-fw"></i>Delete Student</a>
    </div>
...
?>
<div class="quick-nav btn-group">
<a class="btn btn-primary" href="admin.php?dispatcher=courseenrolments&task=add">Add Student</a>
<a class="btn btn-default" href="admin.php?dispatcher=courseenrolments&task=reset">Reset Filters</a>
</div>
<?php } ?>

<?php function showrecnav($a, $recid, $count) { ?>
<div class="quick-nav btn-group">
<a class="btn btn-default" href="admin.php?dispatcher=courseenrolments"><i class="fa fa-undo fa-fw"></i> Back to Students</a>
<?php if ($recid > 0) { ?>
<a class="btn btn-default" href="admin.php?dispatcher=courseenrolments&task=<?=$a ?>&recid=<?=$recid - 1 ?>"><i class="fa fa-arrow-left fa-fw"></i> Prior Record</a>
<?php } if ($recid < $count - 1) { ?>
<a class="btn btn-default" href="admin.php?dispatcher=courseenrolments&task=<?=$a ?>&recid=<?=$recid + 1 ?>"><i class="fa fa-arrow-right fa-fw"></i> Next Record</a>
<?php } ?>
</div>
<?php } ?>
...
function viewrec($recid){
	...
	?>
	<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?dispatcher=courseenrolments">Students</a></li><li class="active">View Student</li></ol>
	<?php 
		...
		if(sizeof($ERRORS) > 0){
			...
			if(sql_insert($FIELDS)){
				...
				if(!$mail->Send()) {
					...
					redirect("admin.php?dispatcher=courseenrolments");
				}else{
					//Display Confirmation Message
...
?>
<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?dispatcher=courseenrolments">Students</a></li><li class="active">Add Student</li></ol>

<a class="btn btn-default" href="admin.php?dispatcher=courseenrolments"><i class="fa fa-undo fa-fw"></i> Back to Students</a>

<p class="text-center"><?php if(sizeof($ERRORS['MSG'])>0) echo $ERRORS['MSG'];?></p>
<form id="validateform" enctype="multipart/form-data" action="admin.php?dispatcher=courseenrolments&task=add" method="post">
...
<input class="btn btn-default" type="button" name="cancel" value="Cancel" onclick="javascript:location.href='admin.php?dispatcher=courseenrolments'" />
</p>
</form>
<?php } ?>
...
?>
<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?dispatcher=courseenrolments">Students</a></li><li class="active">Edit Student</li></ol>
<?php showrecnav("edit", $recid, $count); ?>
<form id="validateform" enctype="multipart/form-data" action="admin.php?dispatcher=courseenrolments&task=edit&recid=<?=$recid?>" method="post">
<p class="text-center"><?php if(sizeof($ERRORS['MSG'])>0) echo $ERRORS['MSG'];?></p>
...
function deleterec($recid){
	...
	$row = db_fetch_array($res);  
?>
<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?dispatcher=courseenrolments">Students</a></li><li class="active">Delete Student</li></ol>
<?php showrecnav("del", $recid, $count); ?>
<form action="admin.php?dispatcher=courseenrolments&task=del&recid=<?=$recid?>" method="post">
...
function manage_announcements($UserType){
	...
	switch ($action) {
		...
		case "edit":
				...
				if(sizeof($ERRORS) > 0){			
					...
					if($action == "add"){
						...
						if(db_affected_rows($conn)>0){
							...
							//Redirect
							redirect("admin.php?dispatcher=courseenrolments#tabs-2");
						}else{
			...
			?>
			<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?dispatcher=courseenrolments">Students</a></li><li><a href="admin.php?dispatcher=courseenrolments#tabs-2">Announcements</a></li><li class="active"><?=ucwords($action)?> Announcements</li></ol>
			<p class="text-center"><?php if(sizeof($ERRORS['MSG'])>0) echo $ERRORS['MSG'];?></p>
			...
			<div class="row">
				...
				<div class="col-md-6">
					<form id="validateform" enctype="multipart/form-data" action="admin.php?dispatcher=courseenrolments&subtab=announcements&action=<?=$action?>&eid=<?=$editID?>#tabs-2" method="post">										
					<div class="form-group">
					...
					<div class="form-group">
					  <div class="text-center"><input class="btn btn-primary" type="submit" name="Save" value="Save">&nbsp;<input class="btn btn-default" type="button" name="cancel" value="Cancel" onclick="javascript:location.href='admin.php?dispatcher=courseenrolments#tabs-2'"></div>
					</div>
					</form>
			  </div>
				<div class="col-md-3"></div>
		...
		<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?dispatcher=courseenrolments">Students</a></li><li class="active">Announcements</li></ol>
		...
		<div id="hideMsg"><?php if(isset($_SESSION['MSG'])) echo $_SESSION['MSG'];?></div>
		<a class="btn btn-primary" href="admin.php?dispatcher=courseenrolments&subtab=announcements&action=add#tabs-2">Add Announcement</a>        
		...
		if(db_num_rows($result)>0){
		  ...
		  while($announce = db_fetch_array($result)){
			  ...
			  <td><a href=\"?dispatcher=courseenrolments&subtab=announcements&action=view&eid=".$announce['UID']."#tabs-2\" title=\"View\">View</a> | <a href=\"?dispatcher=courseenrolments&subtab=announcements&action=edit&eid=".$announce['UID']."#tabs-2\" title=\"Edit\">Edit</a></td>
...
function show_loginhistory(){
	...
	<ol class="breadcrumb"><li><a href="admin.php" title="Dashboard">Dashboard</a></li><li><a href="admin.php?dispatcher=courseenrolments">Students</a></li><li class="active">Login History</li></ol>
