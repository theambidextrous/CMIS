    ...
    <div class="cms-contents-grey">
      ...
      switch($task) {		  
		  ...
		  case "display":
			  ...
			  $From = isset($student['Email'])?$student['Email']:NULL;
			  $FromName = isset($student['StudentName'])?$student['StudentName']:NULL;
			  $Cc = "";
			  $Bcc = "";
			  ...
			  if(!empty($editID)){
				  ...
				  if($task == "reply"){
					  ...
					  if($student['Email'] == $thisMessage['FromAdd']){
						  $To = $thisMessage['ToAdd'];
						  $From = $student['Email'];
					  }else{
			  ...
			  if($task == "new" || $task == "reply" || $task == "replyall"){
				  ...
				  <div class="form-group"> 
					...
					<div class="col-sm-10"><input <?=$ERRORS['To']?> type="text" name="To" value="<?=$To?>" class="form-control" aria-describedby="helpBlockToAdd" style="width:40%;"><span id="helpBlockToAdd" class="help-block"><small>Separate multiple emails with commas e.g. test1@example.com, test2@example.com</small></span></div>
				  </div>
				  <div class="form-group"> 
					<label class="control-label col-sm-2" for="">Cc: </label>
					<div class="col-sm-10"><input <?=$ERRORS['Cc']?> type="text" name="Cc" value="<?=$Cc?>" class="form-control" aria-describedby="helpBlockCcAdd" style="width:40%;"><span id="helpBlockCcAdd" class="help-block"><small>Separate multiple emails with commas e.g. test1@example.com, test2@example.com</small></span></div>
				  </div>
				  <div class="form-group"> 
				  <label class="control-label col-sm-2" for="">Bcc: </label>
					<div class="col-sm-10"><input <?=$ERRORS['Bcc']?> type="text" name="Bcc" value="<?=$Bcc?>" class="form-control" aria-describedby="helpBlockBccAdd" style="width:40%;"><span id="helpBlockBccAdd" class="help-block"><small>Separate multiple emails with commas e.g. test1@example.com, test2@example.com</small></span></div>
				  </div>
				  <div class="form-group"> 
					<label class="control-label col-sm-2" for="">Subject: <span class="text-danger">*</span></label>
					<div class="col-sm-10"><input <?=$ERRORS['Subject']?> type="text" name="Subject" value="<?=$Subject?>" class="form-control" style="width:40%;"></div>
				  </div>
				  <div class="form-group"> 
					<label class="control-label col-sm-2" for="">Attachments: </label>
					<div class="col-sm-10"><input id="msg_attachments" type="file" name="file[]"><small>(Max 5 files)</small></div>
				  ...
				  <div class="form-group">
					<div class="col-sm-12"><textarea id="message" name="Message" class="tinymce"><?=$Message?></textarea></div>
				  </div>
			  ...
			  <div class="btn-group">
			  <a class="btn btn-default" href="?dispatcher=messages&amp;task=view&amp;folder=<?=$folder?>">Back</a>
			  <a class="btn btn-default" href="?dispatcher=messages&amp;task=reply&amp;eid=<?=$editID?>&amp;folder=<?=$folder?>">Reply</a>
			  <a class="btn btn-default" href="?dispatcher=messages&amp;task=replyall&amp;eid=<?=$editID?>&amp;folder=<?=$folder?>">Reply All</a>
			  <a class="btn btn-default" href="?dispatcher=messages&amp;task=delete&amp;eid=<?=$editID?>&amp;folder=<?=$folder?>">Delete</a>
			  </div>
			  </td>
			  </tr>
			  <tr>
		  ...
		  case "view":		
			  ...
			  if(db_num_rows($result)>0){
				  while($message = db_fetch_array($result)){
					  $hid = ($message['DateRead'] == "0000-00-00 00:00:00")?'active bold':'';
					  echo "<tr class=\"$hid\">
