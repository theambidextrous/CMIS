...
class FinstockSMSException extends Exception  {}
...
class FinstockSMS
{
  ...
  public function sendMessage($to_, $message_, $from_ = null, $bulkSMSMode_ = 1, Array $options_ = array())
  {
    if ( strlen($to_) == 0 || strlen($message_) == 0 ) {
      throw new FinstockSMSException('Please supply both to and message parameters');
    }
	...
	if ( in_array($key, $allowedKeys) && strlen($value) > 0 ) {
	  ...
	  throw new FinstockSMSException("Invalid key in options array: [$key]");
	}
      }
    }
	...
	return $responseObject->SMSMessageData->Recipients;
      ...
      throw new FinstockSMSException($responseObject->SMSMessageData->Message);
    }
    
    throw new FinstockSMSException($this->_responseBody);
  }
  ...
  public function createSubscription($phoneNumber_, $shortCode_, $keyword_, $checkoutToken_)
  {
    ...
    if ( strlen($phoneNumber_) == 0 || strlen($shortCode_) == 0 || strlen($keyword_) == 0 || strlen($checkoutToken_) == 0 ) {
      throw new FinstockSMSException('Please supply phoneNumber, shortCode, keyword and checkoutToken');
    }
  ...
  public function deleteSubscription($phoneNumber_, $shortCode_, $keyword_)
  {
    if ( strlen($phoneNumber_) == 0 || strlen($shortCode_) == 0 || strlen($keyword_) == 0 ) {
      throw new FinstockSMSException('Please supply phoneNumber, shortCode and keyword');
    }
  ...
  public function call($from_, $to_)
  {
    if ( strlen($from_) == 0 || strlen($to_) == 0 ) {
      throw new FinstockSMSException('Please supply both from and to parameters');
    }
	...
	return $responseObject->entries;
      ...
      throw new FinstockSMSException($responseObject->errorMessage);
    }
    else
      throw new FinstockSMSException($this->_responseBody);
  }
  ...
  public function bankPaymentCheckoutCharge($productName_,
    ...
    if($this->_responseInfo['http_code'] == self::HTTP_CODE_CREATED) {
      ...
      else throw new FinstockSMSException($response->description);
    }
    throw new FinstockSMSException($this->_responseBody);
  }
  ...
  public function mobilePaymentB2CRequest($productName_,
    ...
    if($this->_responseInfo['http_code'] == self::HTTP_CODE_CREATED) {
      ...
      else throw new FinstockSMSException($response->errorMessage);
    }
    throw new FinstockSMSException($this->_responseBody);
  }
